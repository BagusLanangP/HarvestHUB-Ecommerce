...
@section('tittle', 'Profil Toko - ' . $toko->nama)
            ...
            <div class="col-12 col-lg-5">
                ...
                <div class="card border-0 shadow rounded-4 p-4 mb-4 text-center text-sm-start bg-white shop-profile-card">
                    <div class="d-flex align-items-center gap-3 flex-wrap flex-sm-nowrap justify-content-center justify-content-sm-start">
                        {{-- Logo Toko --}}
                        <img src="{{ $toko->foto ? asset('storage/' . $toko->foto) : asset('img/default-shop.png') }}" class="rounded-circle shadow border border-3 border-white shop-logo-display flex-shrink-0" alt="{{ $toko->nama }}" style="width: 100px; height: 100px; object-fit: cover;">
                        
                        <div>
                            <h4 class="fw-bold text-dark mb-1">{{ $toko->nama }}</h4>
                            {{-- Rating & Bintang --}}
                            <div class="d-flex align-items-center justify-content-center justify-content-sm-start mb-2 gap-1" style="font-size: 0.85rem;">
                                <span class="text-warning">
                                    @for($i=1; $i<=5; $i++)
                                        @if($i <= round($toko->overall_rating))
                                ...
                                <span class="text-secondary fw-semibold">({{ $toko->overall_rating }} / 5)</span>
                            </div>
                            
                            {{-- Button Follow & Chat --}}
                            <div class="d-flex gap-2">
                                <button type="button" class="btn btn-success btn-sm rounded-pill px-3 fw-semibold shadow-sm btn-follow d-inline-flex align-items-center gap-1">
                                    <i class="bi bi-plus"></i> Ikuti
                                </button>   
                                @php
                                    $tokoPhone = preg_replace('/[^0-9]/', '', $toko->phone);
                                    if (str_starts_with($tokoPhone, '0')) {
                                        $tokoPhone = '62' . substr($tokoPhone, 1);
                                    }
                                    $waUrl = "https://wa.me/" . $tokoPhone . "?text=Halo%20Toko%20" . urlencode($toko->nama) . ",%20saya%20tertarik%20dengan%20produk%20tani%20Anda%20di%20HarvestHUB.";
                                @endphp
                                <a href="{{ $waUrl }}" target="_blank" class="btn btn-outline-success btn-sm rounded-pill px-3 fw-semibold btn-chat d-inline-flex align-items-center gap-1 text-decoration-none">
                        ...
                        <table class="table table-borderless mb-0 align-middle shop-info-table" style="font-size: 0.92rem;">
                            <tbody>
                                <tr>
                                    ...
                                    <td class="text-dark fw-medium py-2">{{ $toko->email }}</td>
                                </tr>
                                <tr>
                                    <td class="text-secondary ps-0 py-2">Telepon</td>
                                    <td class="text-dark fw-medium py-2">{{ $toko->phone }}</td>
                                </tr>
                                <tr>
                                    <td class="text-secondary ps-0 py-2">Wilayah</td>
                                    <td class="text-dark fw-medium py-2">{{ $toko->region ?: 'Indonesia' }}</td>
                                </tr>
                                <tr>
                                    <td class="text-secondary ps-0 py-2">Tahun Berdiri</td>
                                    <td class="text-dark fw-medium py-2">{{ $toko->year_started ?: '-' }}</td>
                                </tr>
                                <tr>
                                    <td class="text-secondary ps-0 py-2">Alamat</td>
                                    <td class="text-dark fw-medium py-2">{{ $toko->alamat }}</td>
                                </tr>
                                @if($toko->link_tiktok || $toko->link_ig || $toko->link_fb)
                                <tr>
                                    <td class="text-secondary ps-0 py-2">Media Sosial</td>
                                    <td class="py-2">
                                        <div class="d-flex gap-2">
                                            @if($toko->link_tiktok)
                                                <a href="{{ $toko->link_tiktok }}" target="_blank" class="btn btn-light btn-sm rounded-circle p-0 d-flex align-items-center justify-content-center border" style="width: 32px; height: 32px;" title="TikTok"><i class="bi bi-tiktok text-dark"></i></a>
                                            @endif
                                            @if($toko->link_ig)
                                                <a href="{{ $toko->link_ig }}" target="_blank" class="btn btn-light btn-sm rounded-circle p-0 d-flex align-items-center justify-content-center border" style="width: 32px; height: 32px;" title="Instagram"><i class="bi bi-instagram text-danger"></i></a>
                                            @endif
                                            @if($toko->link_fb)
                                                <a href="{{ $toko->link_fb }}" target="_blank" class="btn btn-light btn-sm rounded-circle p-0 d-flex align-items-center justify-content-center border" style="width: 32px; height: 32px;" title="Facebook"><i class="bi bi-facebook text-primary"></i></a>
                                            @endif
                                        </div>
                                    </td>
                                </tr>
                ...
                <div class="card border-0 shadow rounded-4 p-4 mb-4 bg-white shop-description-card">
                    ...
                    <div class="text-secondary leading-relaxed" style="font-size: 0.92rem;">
                        {!! $toko->deskripsi ?: '<p class="text-muted mb-0">Belum ada deskripsi.</p>' !!}
                    </div>
                </div>

                {{-- Tombol Aksi Pemilik (Edit & Hapus Sejajar) --}}
                @auth
                    @if($toko->user_id == auth()->id() || auth()->user()->role_id == 1)
                        <div class="d-flex align-items-center gap-2 mb-4">
                            {{-- Button Edit --}}
                            <a href="{{ route('Toko.edit', $toko->id) }}" class="btn btn-warning rounded-pill flex-grow-1 fw-semibold py-2 d-inline-flex align-items-center justify-content-center gap-1 btn-owner shadow-sm">
                                <i class="bi bi-pencil-square"></i> Edit Profil
                            </a>
                            
                            {{-- Button Hapus --}}
                            <form action="{{ route('Toko.destroy', $toko->id) }}" method="POST" class="flex-grow-1" onsubmit="return confirm('Apakah Anda yakin ingin menghapus toko ini secara permanen?')">
                                @csrf
                                @method('DELETE')
...
<style>
    ...
    .shop-profile-card, .shop-info-card, .shop-description-card, .shop-products-card {
